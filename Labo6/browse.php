...
if (isset($_POST['moduleAction']) && ($_POST['moduleAction'] == 'add')) {
    ...
    if (sizeof($formErrors) == 0) {
        ...
        $stmt = $db->prepare('INSERT INTO todolist (what, priority, added_on, userID) VALUES (?, ?, ?, ?)');
        $stmt->execute(array($what, $priority, (new DateTime())->format('Y-m-d H:i:s'), $_SESSION['userID']));
...
$stmt = $db->prepare('SELECT * FROM todolist WHERE userID = ? ORDER BY priority, what DESC ');
