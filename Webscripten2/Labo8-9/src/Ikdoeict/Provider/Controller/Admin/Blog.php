    ...
    public function edit(Application $app, $blogpostId) {
        ...
        if ('POST' == $app['request']->getMethod()) {
            ...
            if ($editform->isValid()) {
                ...
                // Update data in DB
                $app['db.blog']->update($editform->getData(), array('id' => $blogpostId));
    ...
    public function add(Application $app) {
        ...
        if ('POST' == $app['request']->getMethod()) {
            ...
            if ($addform->isValid()) {
                ...
                var_dump($files['images']); die();

                /*
                // Uploaded file must be `.jpg`!
                if (isset($files['images']) && ('.jpg' == substr($files['images']->getClientOriginalName(), -4))) {
                    // Move it to its new location
                    $files['images']->move($app['cms.base_path'] . $id, time().'-'. $files['images']->getClientOriginalName());
